...
\begin{abstract}
    ...
    We introduce a tree analogue $\mathfrak{at}$ of $\mathfrak{ap}$ and prove $\mathfrak q_1\leq\mathfrak{at}\leq\mathfrak q_{2\frac{1}{2}}$, hence $\mathfrak{ap}\leq\mathfrak q_{2\frac{1}{2}}$.
    Assuming the Generalized Continuum Hypothesis, we construct ccc forcing extensions with $\mathfrak{ap}=\omega_1<\mathfrak{at}=\mathfrak q_{2\frac{1}{2}}=\mathfrak c$, so $\mathfrak{ap}<\mathfrak{at}$ is consistent with ZFC.
