...
\begin{prob}[{\cite[Problem 11]{banakh2023q}}] Is $\mathfrak{ap}\leq\mathfrak q_2$?
\end{prob}

This problem remains open, but we prove several related bounds.
In Section~\ref{sec:urysohn-q-spaces}, we introduce a tree analogue of
...
$\mathfrak{ap}\leq\mathfrak{at}$, and we prove
\[
    \mathfrak q_1\leq\mathfrak{at}\leq \mathfrak q_{2\frac{1}{2}}.
\]
In particular, this gives the ZFC bound
\[
    \mathfrak{ap}\leq \mathfrak q_{2\frac{1}{2}}.
\]

The proof of the Urysohn bound shows that the extra separation available in
Urysohn spaces can be encoded by almost disjoint subtrees. In
Section~\ref{sec:small-diagonals}, we isolate another form of this extra
structure by considering Hausdorff spaces with $G_\delta$-diagonals. Let
$\mathfrak q_{2\Delta}$ be the least cardinality of a second-countable
Hausdorff non-$Q$-space with a $G_\delta$-diagonal. We prove
\[
    \mathfrak{ap}\leq \mathfrak q_{2\Delta}.
\]
Thus, every second-countable Hausdorff space of cardinality $<\mathfrak{ap}$
with a $G_\delta$-diagonal is a $Q$-space. We also record that every
second-countable Urysohn space has a $G_\delta$-diagonal, and that every
second-countable $T_i$ space of cardinality $<\mathfrak q_i$, for
$i\in\{1,2\}$, has a $G_\delta$-diagonal.
...
\]
In particular, $\mathfrak{ap}<\mathfrak{at}$ is consistent.
The construction is based on Brendle's model for $\mathfrak{ap}<\mathfrak q$
and uses forcing notions associated with almost disjoint families of subtrees.
